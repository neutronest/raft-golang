...
func TestReElection2A(t *testing.T) {
	...
	cfg.checkOneLeader()
    fmt.Println("[===]Passed: if the leader disconnects, a new one should be elected.\n")

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
    fmt.Println("[===]Passed: if the old leader rejoins, that shouldn't disturb the old leader\n")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
    fmt.Println("[===]Passed-small:  if there's no quorum, no leader should be elected \n")

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
    fmt.Println("[===]Passed-small: if a quorum arises, it should elect a leader.")


	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
    fmt.Println("[===]Passed-small: re-join of last node shouldn't prevent leader from existing.")
	fmt.Printf("  ... Passed\n")
}
...
func TestFailAgree2B(t *testing.T) {
    ...
    fmt.Println("=== vote rpc : one disconnected server")
	cfg.disconnect((leader + 1) % servers)

	// agree despite one disconnected server?
	fmt.Println("=== agree despite one disconnected server?")
	...
	fmt.Println("=== agree with full set of servers?")
	cfg.one(106, servers)
	time.Sleep(RaftElectionTimeout)
	cfg.one(107, servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	fmt.Printf("=== 3 of 5 followers disconnect\n")
	leader := cfg.checkOneLeader()
	cfg.disconnect((leader + 1) % servers)
	...
	for try := 0; try < 5; try++ {
		...
		for j := 0; j < servers; j++ {
			if t, _ := cfg.rafts[j].GetState(); t != term {
                ...
                fmt.Println("term changed -- can't expect low RPC counts")
				continue loop
			}
		}
...
func TestBackup2B(t *testing.T) {
	...
	fmt.Printf("=== put leader and one follower in a partition\n")
	leader1 := cfg.checkOneLeader()
	...
	for i := 0; i < entryNum; i++ {
		...
		cfg.rafts[leader2].Start(i+ 2*entryNum)
	}
